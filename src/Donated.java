...
public class Donated{
	 ...
	 */
	public Donated(int donationId, int donorId, Date donationDate){
	...
	public int save(){
		...
		try {
			...
			String sql = "insert into donated(donationId, donorId, donationDate) values(?, ?, ?);";
	 ...
	 */
	public static Donated open(int donationId){
		Connection c = BetaUniversity.getConnection();
		PreparedStatement stmt = null;
		...
		try {
			...
			String sql = "select donorId, donationDate from donated where donationId = ?;";
			stmt = c.prepareStatement(sql);
